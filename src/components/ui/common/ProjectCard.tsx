            ...
            <div className={`flex lg:cgap-8 flex-col  //border-2 bg- rounded-lg hover:shadow-lg hover:shadow-blue-shade/50 duration-200 text-grey ${pathname === projects ? "lg:flex-col" : "lg:flex-row"}`}>
                <div className={`${pathname === projects ? " rounded-tl-lg rounded-tr-lg " : "md:pt-10 md:px-7 lg:rounded-tl-lg lg:rounded-bl-lg lg:rounded-tr-none"}  flex flex-col justify-end items-center border-2 border-secondary`}>
                    <div className={` gap-1 py-3 hidden flex-wrap ${pathname === projects ? "hidden" : "md:flex"}`}>
                        ...
                        {props.featured.stacks.map((icon) => (
                            <span
                                key={icon}
                ...
                <div className={`"//shadow-md shadow-[#d1d1d1]/40 p-5   bg-secondary rounded-bl-lg rounded-br-lg rounded-tr-none rounded-tl-none" ${pathname === projects ? "" : "md:p-10 lg:rounded-tr-lg lg:rounded-br-lg lg:rounded-tl-none lg:rounded-bl-none"}`}>
                    ...
                    <p>{props.featured.desc[0]}</p>
                    {/* {props.featured.tasks.length > 0 && (<h4 className="text-[1.1rem] font-[600]">Main Features:</h4>)}
                    <ul className="px-2 list-disc list-inside ">{props.featured?.tasks.map(task => (<li key={task}>{task}</li>))}</ul> */}
                    ...
                    <div className={`flex justify-between w-full items-center mt-4 md:mt-5 lg:mt-24 bottom-1`}>
                        <div className={`flex justify-between`}>

                            <Link href={`/projects/${props.featured?.slug}`} className={`flex items-center text-soft-white  hover:text-primary-blue duration-200 border-b border-soft-white hover:border-primary-blue /py-1`}>Details <Icon icon="basil:arrow-right-solid" width="26" height="26" /></Link>
                        </div>
                        <div className={`flex justify-between gap-2 md:gap-3 items-center /mt-5 md:items-end //flex-col flex-row /w-full ${pathname === projects ? "" : "md:justify-end"}`}>
                            {props.featured.codeLink && <Link
                                href={props.featured.codeLink}
                            ...
                            <Link href={props.featured.liveLink} className={`px-1 py-2   duration-200 text-soft-white  hover:text-primary-blue ${pathname === projects ? "" : "md:hidden"}`}>
                            ...
                            <Link href={props.featured.liveLink} className={`px-1 py-2 bg-primary-blue rounded-md text-soft-white hover:bg-transparent hover:text-primary-blue border-primary-blue border  duration-200 hidden ${pathname === projects ? "" : "md:inline-block"}`}>
                                <button className="w-[7rem] flex items-center gap-3 md:gap-4 justify-center">
